 ...
 */
export async function previewFinalsAdvancement(tournamentId, finalTeamCount = DEFAULT_FINAL_TEAM_COUNT) {
  ...
  return buildFinalsAdvancementPreview(schedule, resultsMap, finalTeamCount);
}
 ...
 * @param {number} [finalTeamCount]
 */
export async function saveFinalsAdvancement(tournamentId, finalTeamCount = DEFAULT_FINAL_TEAM_COUNT) {
  ...
  const preview = await previewFinalsAdvancement(tournamentId, finalTeamCount);
  ...
  const payload = {
    ...buildPersistedFinalsAdvancement(preview),
